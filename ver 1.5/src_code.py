...
class AppInit:        
    ...
    def CreateWindows():
        ...
        with dpg.window(label="Home",width=winw,
            ...
            with dpg.menu_bar():
                with dpg.menu(label="File", tag='file'):
                    ...
                    dpg.add_menu_item(label="Clear Queue", callback=lambda: Helper.clearqueue())
                ...
                dpg.add_button(label="Options",callback=lambda:Helper.OptionShow())
        ...
        with dpg.window(label="Logs History", width=800, height=500, 
            ...
            dpg.add_button(label="Back",callback=lambda:Helper.HideLog())
    ...
    def init_viewport_spec():
        dpg.create_viewport(title='Sortie V1.5', width=winw, height=winh, resizable=False)
        dpg.setup_dearpygui()
        dpg.show_viewport()
...
class Helper:
    ...
    def on_sharp_selected(sender,app_data):
        ...
        print(f"Selected file full path: {full_path}")
    ...
    @staticmethod
    def OptionShow():
        dpg.show_item("option_window")
        dpg.focus_item("option_window")
    ...
    @staticmethod
    def next_click():
        Helper.writeToLog("Next button Clicked")
        Helper.start_main()
    ...
    @staticmethod
    def HideLog():
        dpg.configure_item("log_window", show=False)
        dpg.set_primary_window("log_window",False)
    ...
    def Focus_compute(focus_score):
        ...
        return False
    @staticmethod    
    def gaussian_score(sharpness_level, mean=1.0, std_dev=0.5):
        return math.exp(-((sharpness_level - mean) ** 2) / (2 * std_dev ** 2))

    @staticmethod    
    def calculate_focus_score(subjects):
        max_image_score = 100
        human_weight = 1.5  # Weight for human subjects (adjust this later)

        total_score = 0
        total_possible_score = 0

        for subject in subjects:
            sharpness_level = subject['sharpness_level']
            is_human = subject['is_human']
            
            score = Helper.gaussian_score(sharpness_level)
            
            if is_human:
                score *= human_weight
            
            total_score += score
            total_possible_score += Helper.gaussian_score(1.0) * (human_weight if is_human else 1)
